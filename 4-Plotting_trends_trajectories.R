...
mod_load <- "_NB"
mod <- paste(mod_load,LYYYY_trend,sep = "_")
# out_base <- paste0(species_f,mod_load)
...
for(sp in sps){
  ...
  if(file.exists(paste0(output_dir,"/",out_base,"_",LYYYY,"_fit.RData"))){
    ...
    Nsamples <- posterior_samples(fit = stanfit,
                                  parm = "N_alt",
                                  dims = c("y"))
    ...
    NSmoothsamples <- posterior_samples(fit = stanfit,
                                        parm = "NSmooth_alt",
                                        dims = c("y"))
    NSmoothsamples$year <- NSmoothsamples$y + (syear-1)
    
    # loo_ic[[sp]] <- loo(stanfit)
    #  
    # # # looic -------------------------------------------------------------------
    # # 
    # # 
    ...
    alpha_samples <- posterior_samples(fit = stanfit,
                                       parm = "alpha",
                                       dims = c("s"))
    
    sdalpha_samples <- posterior_samples(fit = stanfit,
                                         parm = "sdalpha") 
    ...
    ALm <- posterior_samples(fit = stanfit,
                             parm = "ALPHA1",
                             dims = NULL) %>% 
      summarise(mean = mean(.value)) %>% 
      as.numeric()
    ...
    sdnm <- posterior_samples(fit = stanfit,
                              parm = "sdnoise",
                              dims = NULL) %>% 
      summarise(mean = mean(.value)) %>% 
      as.numeric()
    ...
    if(grepl(x = mod.file,pattern = "two_season")){
      ...
      season_samples <- posterior_samples(fit = stanfit,
                                          parm = "season_pred",
                                          dims = c("d","s"))
      ...
      pdf(file = paste0("Figures/",sp,mod,"_cmd_simple_Season.pdf"),
          width = 8.5,
          height = 8.5)
      ...
      pdf(file = paste0("Figures/",sp,mod,"_cmd_simple_Season_simplified.pdf"),
          width = 8.5,
          height = 8.5)
      print(pp_simple)
      dev.off()
    ...
    AA_y_p <- ggplot(data = alphas_yr,aes(x = YearCollected,y = mean_alpha))+
      ...
      xlab("")+
      labs(title = paste(sp,mod,"_mean site-effect of included sites by year"))
    
    pdf(file = paste0("Figures/",sp,mod,"_cmd_alphas_by_yr.pdf"),
        width = 8.5,
        height = 8.5)
    print(AA_y_p)
    ...
    tdif <- ItoTT_comparison(inds = NSmoothsamples,
                             starts = c(syear,y3g),
                             ends = c(LYYYY_trend,LYYYY_trend),
    ...
    #    
        ...
        nadd = length(syear:LYYYY_trend)-length(stg)
        if(nadd > 0){
          stg <- c(stg,rep("earlier",nadd))
        }
        if(nadd < 0){
          stg <- stg[1:length(syear:LYYYY_trend)]
        }


        stag <- data.frame(year = syear:LYYYY_trend,
                           stage = rev(stg))
    #     
    #     
    #     
    ...
    #     
        if(grepl(x = mod.file,pattern = "two_season")){
          ...
          seas = posterior_samples(stanfit,
                                   parm = "season_pred",
                                   dims = c("d","r")) %>%
            posterior_sums(.,
                           quantiles = NULL,
        ...
        yup = max(max(indices$uci),quantile(indices$obsmean,0.7,na.rm = TRUE))

        indices$parm <- factor(indices$parm,ordered = T,levels = c("NSmooth_alt","N_alt"))

        N_gg = ggplot(data = indices,aes(x = year, y = median,fill = parm))+
          geom_ribbon(aes(ymin = lci,ymax = uci),alpha = 0.2)+
          geom_line(aes(colour = parm))+
          labs(title = paste(sp,mod,"_Survey-wide trajectory (full and smooth) with obs means"))+
        ...
        pdf(paste0("figures/",sp,mod,FYYYY,"_cmd_GAMYE_survey_wide_trajectory_simple.pdf"))
        print(N_gg)
        dev.off()
        ...
        N_gg_simple = ggplot(data = indices,aes(x = year, y = median,fill = parm))+
          geom_ribbon(aes(ymin = lci,ymax = uci),alpha = 0.2)+
          geom_line(aes(colour = parm))+
          labs(title = paste(sp,mod,"_Survey-wide trajectory (full and smooth)"))+
          my_col2_traj+
          xlab("")+
    ...
    # Continental Smooth spaghetti plot ---------------------------------------
      #  indicesNSmooth$year = indicesNSmooth$year + (syear-1)
        ...
        n_gg_spag = ggplot(data = indicesNSmooth,aes(x = year, y = median))+
          ...
          geom_line(size =2)+
          labs(title = paste(sp,mod,"_Random selection of 100 posterior draws of survey-wide trajectories"),
               subtitle = paste0("Colour of each posterior draw reflects the value in ","LYYYY"," demonstrating similar smooths across draws"))+
          xlab("")+
          ylab("Modeled mean count")+
          theme_classic()+
          theme(legend.position = "none") +
    ...
    # pdf(paste0("figures/",sp,mod,FYYYY,"_GAMYE_survey_wide_trajectory_simple.pdf"))
    # print(N_gg_simple)
    # dev.off()
    ...
    #     
        ...
        indices_strat <- left_join(indices_strat,strats_dts, by = c("s" = "stratn"))
        ...
        obs_counts_st <- rawdat %>%
          group_by(year,stratn) %>%
          summarise(obsmean = mean(count_scale),
                    n_counts = n(),
                    sqrt_n = sqrt(n_counts),
                    .groups = "keep")
        ...
        indices_strat = left_join(indices_strat,obs_counts_st,
                            by = c("year",c("s" = "stratn")))

    #     
    #     
    #     pdf(file = paste0("figures/", sp,mod,FYYYY,"_cmd_GAMYE_Strata_trajectories_simple.pdf"),
    #         width = 8.5,
    #         height = 11)
    #     print(N_gg)
    #     # ncl = 3
    ...
    #     # }
        ...
        for(jj in 1:ceiling(nstrata/ppag)){
          n_gg_simple = ggplot(data = indices_strat,aes(x = year, y = median,fill = parm))+
            ...
            theme(legend.position = "none")+
            labs(title = sp)+
            facet_wrap_paginate(facets = ~hex_name,page = jj,nrow = nrr, ncol = ncl,scales = "free")
    ...
    # 
        ...
        #     theme(legend.position = "none")+
        #   scale_y_log10()
        # pdf(paste0("Figures/log_scale_trajectories_",sp,mod,"_cmd_.pdf"),
        #     width = 11,
        #     height = 8.5)
        # print(n_gg_over)
        # dev.off()
    ...
    #     #                           dat = B)
    #       
    #     B_samples <- posterior_samples(fit = stanfit,
    #                             parm = "B",
    #                             dims = c("k"))
    #     b_samples <- posterior_samples(fit = stanfit,
    #                      parm = "b",
    ...
    #       left_join(.,strats_dts,by = c("s" = "stratn"))
    #     
    #     sdbeta_samples <- posterior_samples(fit = stanfit,
    #                                  parm = "sdyear_gam_strat",
    #                                  dims = c("k")) 
    #     
    #     sdB_samples <- posterior_samples(fit = stanfit,
    #                                   parm = "sdyear_gam",
    #                                   dims = NULL) 
    #     
    #     
    ...
    #     
    #     
    #     pdf(paste0("Figures/beta_sdbeta_",sp,mod,"_cmd_.pdf"),
    #         width = 11,
    #         height = 8.5)
    #     print(b_plot)
    #     dev.off()
    ...
    pdf(paste0("Figures/Trend_Heat_maps_",sp,mod,"_cmd_.pdf"),
        width = 11,
        height = 8.5)
...
write.csv(trendsout,paste0("trends/All_region_strata_composite_trends",FYYYY,mod,".csv"),row.names = FALSE)
...
write.csv(trendsoutsplit[[1]],paste0("trends/All_strata_level_trends",FYYYY,mod,".csv"),row.names = FALSE)
write.csv(trendsoutsplit[[2]],paste0("trends/All_region_level_trends",FYYYY,mod,".csv"),row.names = FALSE)

write.csv(TRENDSout,paste0("trends/All_survey_wide_trends",FYYYY,mod,".csv"),row.names = FALSE)

write.csv(Trend_difout,paste0("trends/All_survey_wide_Differences_in_trends",FYYYY,mod,".csv"),row.names = FALSE)

write.csv(indices_out,paste0("trends/All_survey_wide_trajectories",FYYYY,mod,".csv"),row.names = FALSE)
write.csv(indices_out_strat,paste0("trends/All_strata_level_trajectories",FYYYY,mod,".csv"),row.names = FALSE)
...
save(list = c("trend_maps_1980",
              ...
              "season_graphs",
              "loo_ic"),
     file = paste0("Figures/All_stored_maps",FYYYY,mod,".RData"))
...
# Trend maps --------------------------------------------------------------
pdf(file = paste0("Figures/All_trend_maps_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
...
# Season Graphs --------------------------------------------------------------
pdf(file = paste0("Figures/All_Season_graphs_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
...
pdf(file = paste0("Figures/All_imple_Season_graphs_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
...
pdf(file = paste0("Figures/All_Continental_Trajectories_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
...
pdf(file = paste0("Figures/All_Strata_Trajectories_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
...
pdf(file = paste0("Figures/All_Composite_Trajectories_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
...
pdf(file = paste0("Figures/All_Diagnostic_Continental_Trajectories_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
...
pdf(file = paste0("Figures/All_Diagnostic_Strata_Trajectories_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
...
pdf(file = paste0("Figures/All_alphas_by_yr_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
...
pdf(file = paste0("Figures/All_long_short_term_trends_",FYYYY,mod,".pdf"),
    height = 9,
    width = 6.5)
print(lt_tplot)
dev.off()
...
pdf(file = paste0("Figures/First_last_15Year_trends_",FYYYY,mod,".pdf"),
    height = 9,
    width = 6.5)
print(FL15_tplot)
dev.off()
...
pdf(file = paste0("Figures/All_early_recent_3generation_trends_",FYYYY,mod,".pdf"),
    height = 9,
    width = 6.5)
print(el_tplot)
dev.off()
...
pdf(file = paste0("Figures/All_long_term_3Gen_trends_",FYYYY,mod,".pdf"),
    height = 9,
    width = 6.5)
print(lt3_tplot)
dev.off()
...
pdf(file = paste0("Figures/All_beta_plots_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
  if(!is.null(sp_ind_plots_strat_diagnostic[[sp]])){
...
pdf(file = paste0("Figures/All_trajectory_plots_",FYYYY,mod,".pdf"),
    width = 9, height = 6.5)
for(sp in sps){
  if(!is.null(sp_ind_plots_strat_diagnostic[[sp]])){
...
pdf(file = paste0("Figures/All_Differences_in_trends_long_term_3Gen_trends_",FYYYY,mod,".pdf"),
    height = 9,
    width = 6.5)
print(dif_tplot)
dev.off()
...
pdf(file = paste0("Figures/All_Differences_in_trends_all_previous_3Gen_trends_",FYYYY,mod,".pdf"),
    height = 9,
    width = 6.5)
print(dif_tplot2)
dev.off()
...
pdf(file = paste0("Figures/All_Differences_in_trends_early_Vs_late_3Gen_trends_",FYYYY,mod,".pdf"),
    height = 9,
    width = 6.5)
print(dif_tplot3)
dev.off()
...
TRENDSout <- read_csv(paste0("trends/All_survey_wide_trends",1980,"_NB_2024",".csv"))

indices_out <- read_csv(paste0("trends/All_survey_wide_trajectories",1980,"_NB_2024",".csv"))

trend_share <- TRENDSout %>% 
  mutate(across(trend:mean_n_surveys, ~ signif(.x,3)))

write_csv(trend_share,"trends/All_Shorebird_Migration_Survey_Trends_2024.csv")

inds_share <- indices_out %>% 
  select(-c(y,mean,stage)) %>% 
  mutate(smooth_or_full_indices = ifelse(parm == "N_alt","Full","Smooth"),
         across(median:uci, ~ signif(.x,3))) %>% 
  rename(predicted_mean_abundance = median,
         lower_95percent_CL = lci,
         upper_95percent_CL = uci)
  
write_csv(inds_share,"trends/All_Shorebird_Migration_Survey_Indices_2024.csv")
