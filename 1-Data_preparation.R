...
acss_sites <- read_csv("data/acss/first_draft_coord_name_province_by_sitecode.csv")
...
all_sites <- ssData %>% group_by(SurveyAreaIdentifier,DecimalLatitude,DecimalLongitude) %>% 
  ...
  iss_sites_laea <- st_join(iss_sites_laea, poly_regs, join = st_nearest_feature)
  
  
  # Adding coarse region information- geographic overlay ----------------------------------

  # iss_sites_regs <- iss_sites_laea %>% 
  #   st_join(regions, join = st_nearest_feature) # %>% 
  # st_drop_geometry()
  # 
  
  tst_plot <- ggplot()+
    geom_sf(data = poly_regs,
            aes(colour = Region))+
 ...
 tst_plot 
  ...
  saveRDS(iss_sites_regs, "Data/site_map.rds")
  
  strats <- iss_sites_regs %>% 
    select(-c(Id,n_years))
  ...
  saveRDS(regions,"Data/regional_map.rds")
