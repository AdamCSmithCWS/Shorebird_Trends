...
for(sp in sps){
  ...
  if(file.exists(paste0(output_dir,"/",out_base,"_",LYYYY,"_fit.RData"))){
    ...
    if(grepl(x = mod.file,pattern = "two_season")){
      ...
      pdf(file = paste0("Figures/",sp,"_cmd_simple_Season_simplified.pdf"),
          width = 8.5,
          height = 8.5)
      print(pp_simple)
      dev.off()
    ...
    #     
        ...
        pdf(paste0("figures/",sp,FYYYY,"_cmd_GAMYE_survey_wide_trajectory_simple.pdf"))
        print(N_gg)
        dev.off()
    ...
    pdf(paste0("Figures/Trend_Heat_maps_",sp,"_cmd_.pdf"),
        width = 11,
        height = 8.5)
