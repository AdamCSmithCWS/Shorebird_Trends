...
transformed parameters { 
  ...
  for(i in 1:ncounts){
    ...
    E[i] = year_pred[year_raw[i],strat[i]] + alpha[site[i]] + year_effect[year_raw[i]] + season_pred[date[i]] + noise;
  }
 ...
 // log_lik calculation for looic
  ...
  for(y in 1:nyears){
        ...
        for(j in 1:nsites_strat[s]){
          atmp[j] = exp(year_pred[y,s] + year_effect[y] + seas_max + alpha[sites[j,s]] + 0.5*(sdnoise^2));
          atmp_smo[j] = exp(year_pred[y,s] + seas_max + 0.5*(sdyear^2) + alpha[sites[j,s]] + 0.5*(sdnoise^2));
        }
        n[s,y] = mean(atmp);
    ...
    for(y in 1:nyears){
      ...
      N[y] = exp(ALPHA1 + Y_pred[y] + year_effect[y] + seas_max + 0.5*(sdnoise^2) );
      NSmooth[y] = exp(ALPHA1 + Y_pred[y] + seas_max + 0.5*(sdyear^2) + 0.5*(sdnoise^2) );
      
      N_alt[y] = exp(ALPHA1 + Y_pred[y] + year_effect[y] + seas_max + 0.5*(sdnoise^2) + 0.5*(sdalpha^2));
      NSmooth_alt[y] = exp(ALPHA1 + Y_pred[y] + seas_max + 0.5*(sdyear^2) + 0.5*(sdnoise^2)  + 0.5*(sdalpha^2));
